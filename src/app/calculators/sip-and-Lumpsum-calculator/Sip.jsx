...
const Sip = () => {
  ...
  const handleSip = () => {
    ...
    const inTf = Number(returnRate) / 12;
    const r = inTf / 100;
    ...
    const TotalInvestedT = P * n;

    const TotalInvested = (P * n).toLocaleString("en-IN", {
      maximumFractionDigits: 0,
    });

    const PV = ((P * (Math.pow(1 + r, n) - 1)) / r) * (1 + r);
    const EstimatedReturns = (PV - TotalInvestedT).toLocaleString("en-IN", {
      maximumFractionDigits: 0,
    });
    console.log(EstimatedReturns);

    const MaturityValue = PV.toLocaleString("en-IN", {
      maximumFractionDigits: 0,
    });
